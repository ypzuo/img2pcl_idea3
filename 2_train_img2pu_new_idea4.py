...
# this version modified from train_lm_img2ae.py
from utils.encoder_2 import *
parser = argparse.ArgumentParser()
# the new version mainly modified the point cloud generation part, which is based on the 2_train_gen_pa_up2_idea1.py
...
NUM_POINTS = 1024
GT_PCL_SIZE = 4096
NUM_EVAL_POINTS = 4096				# Number of points predicted
...
def fetch_batch(models, indices, batch_num, batch_size):
	...
	for ind in indices[batch_num*batch_size:batch_num*batch_size+batch_size]:
		...
		pcl_path = join(FLAGS.data_dir_pcl, model_path, '00.npy')
...
if __name__ == '__main__':
	...
	pcl_gt = tf.placeholder(tf.float32, shape=(BATCH_SIZE, GT_PCL_SIZE, 3), name='pcl_gt')
	...
	with tf.variable_scope('generator') as scope:
		...
		up_l2_points, l1_points, _ = get_local_features(reconstr, is_training=True, scope=scope,
						reuse=None, use_normal=False, use_bn=False, use_ibn=False,
						bn_decay=bn_decay,up_ratio=UP_RATIO)
						
		net = concat_features4(features, up_l2_points, l1_points, global_feature, is_training, scope, reuse=None, 
				bn_decay=None,)
	...
	reconstr_img = tf.reshape(outputs, (BATCH_SIZE, GT_PCL_SIZE, 3))
	...
	pu_vars = [var for var in tf.global_variables() if 'generator' in var.name]
	train_vars = psgn_vars + pu_vars
	#train_vars = psgn_vars
	...
	with tf.Session(config=config) as sess:
		...
		load_pointnet_ae(pointnet_ae_logs_path, pu_vars, sess, FLAGS)
